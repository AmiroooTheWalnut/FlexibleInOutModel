 ...
 * See T. Kohonen, Self-Organization and Associative Memory, 3rd Edition,
 * Springer, 1989
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class SelfOrganizingMap extends AbstractClusterer
        implements OptionHandler, UpdateableClusterer {
    ...
    /**
     * This flag states that the user wants to calculate statistics after
     * training.
     */
    ...
    /**
     * holds the number of instances passed from a node ADDED BY AMIROOO
     */
    public int[] recordInClusters;
    /**
     * holds the attributes as header ADDED BY AMIROOO
     */
    public ArrayList<Attribute> header = new ArrayList();
    /**
     * the number of data to regrenerate ADDED BY AMIROOO
     */
    private int numRegenerate = 100;
    ...
    /**
     * Sets the height of lattice. Must be greater than 0.
     *
    ...
    /**
     * Set the number of epochs in convergence phase. Must be greater than or
     * equal to 1000.
     *
    ...
    /**
     * Set the number of epochs in ordering phase. Must be greater than or equal
     * to 2000.
     *
     * @param n The number of epochs.
     */
    ...
    /**
     * The initial learning rate can be set using this command. Must be greater
    ...
    /**
     * @return a string to describe the number of epochs in ordering phase
     * option.
     */
    ...
    private void calcStatistics() {
        header.clear();//ADDED B Y AMIROOO
        for (int i = 0; i < m_instances.numAttributes(); i++) {//ADDED B Y AMIROOO
            header.add(m_instances.attribute(i));//ADDED B Y AMIROOO
        }//ADDED B Y AMIROOO
        ...
        recordInClusters = new int[m_clusters.numInstances()];//ADDED B Y AMIROOO

        //calc min, max, mean, stdev for each cluster
        for (int cluster = 0; cluster < m_clusters.numInstances(); cluster++) {
            recordInClusters[cluster] = m_clusterInstances[cluster].numInstances();//ADDED B Y AMIROOO
     ...
     * @param winningCluster the winning cluster for the current training
     * instance
    ...
    /**
     * This function calculates the effective width of the topological netowork
    ...
    /**
     * This function performs the denormalization of the attributes of an
     * instance.
     *
    ...
    /**
     * This function performs the normalization of the attributes of an
     * instance.
     *
     * @param inst the instance.
     * @return The modified instance. This needs to be done as it deep copies
    ...
    /**
     * This function returns the clusters if the clusterer is build or an
     * exception if the clusterer is not build.
     *
     * The clusters are returned dernomalized even if normalizeAttributes option
     * is set.
     *
     * @return The clusters
     * @throws Exception
    ...
    /**
     * This function returns the training statistics in a 3-dimension array as
     * follows:<br/>
     * First dimension: the attribute index<br/>
     * Second dimension: the cluster index<br/>
     * Third dimension: the static index (Valid values are 0: min, 1: max, 2:
     * mean, 3: st. dev.)
     *
    ...
    /**
     * This function returns the cluster assignment for each of the training
     * instances. The array's index indicates the corresponding cluster.
     *
    ...
    private Instances generateDataFromNet() {
        Instances generatedData = new Instances("generatedData", header, 0);
        double[] numberForEachNode = new double[recordInClusters.length];
        double sumData = 0;
        for (int i = 0; i < recordInClusters.length; i++) {
            sumData = sumData + recordInClusters[i];
        }
        if (sumData > numRegenerate) {
            double divisionRatio = sumData / numRegenerate;
            for (int i = 0; i < recordInClusters.length; i++) {
                numberForEachNode[i] = Math.floor(recordInClusters[i] / divisionRatio);
            }
        } else {
            for (int i = 0; i < recordInClusters.length; i++) {
                numberForEachNode[i] = recordInClusters[i];
            }
        }
        for (int i = 0; i < numberForEachNode.length; i++) {
            for (int j = 0; j < numberForEachNode[i]; j++) {
                Instance inst = new DenseInstance(header.size());
                for (int k = 0; k < header.size(); k++) {
                    if (header.get(k).isNumeric()) {
                        Random ran = new Random();
                        double nxt = ran.nextGaussian();
                        double variance = m_clusterStats[k][i][3];
                        if (Double.isNaN(variance) == true) {
                            variance = 0;
                        }
                        double newData = (nxt * variance) + m_clusterStats[k][i][2];
                        if (newData < m_clusterStats[k][i][0]) {
                            newData = m_clusterStats[k][i][0] - variance/10;
                        }
                        if (newData > m_clusterStats[k][i][2]) {
                            newData = m_clusterStats[k][i][2] + variance/10;
                        }
                        inst.setValue(k, newData);
                    }
                    if (header.get(k).isNominal()) {
                        Random ran = new Random();
                        double nxt = ran.nextGaussian();
                        double variance = m_clusterStats[k][i][3];
                        if (Double.isNaN(m_clusterStats[k][i][3]) == true) {
                            variance = 0;
                        }
                        double newData = (nxt * variance) + m_clusterStats[k][i][2];
                        if (newData < m_clusterStats[k][i][0]) {
                            newData = newData + (m_clusterStats[k][i][0] - newData) / variance;
                        }
                        if (newData > m_clusterStats[k][i][0]) {
                            newData = newData - (newData - m_clusterStats[k][i][0]) / variance;
                        }
                        newData = Math.round(newData);
                        if (newData < 0) {
                            newData = 0;
                        }
                        if (newData > header.get(k).numValues()) {
                            newData = header.get(k).numValues();
                        }
                        inst.setValue(k, newData);
                    }
                }
                generatedData.add(inst);
            }
        }
...
//            }
        ...
        for (int i = 0; i < second.numAttributes(); i++) {
            if (!newAttributes.contains(second.attribute(i))) {
        ...
        for (int i = 0; i < first.numInstances(); i++) {
            merged.add(first.instance(i));
        }
        for (int i = 0; i < second.numInstances(); i++) {
            merged.add(second.instance(i));
        }
        return merged;
    }
    ...
    public void updateClusterer(Instance instnc) throws Exception {
        Instances generatedData = generateDataFromNet();

        Instances newData = new Instances("generatedData", header, 0);
        newData.add(instnc);

        Instances data = mergeInstances(generatedData, newData);
